...
# Ping pong back upp program.

screen= turtle.Screen()                         #Home page
...
def selection_c(): # Vs Computer mode game - still working on it. (there are some faults on the program -many-)
    ...
    computer_pad.color("black")
    computer_pad.shapesize(stretch_wid=5, stretch_len=1)
    computer_pad.goto(240, 0) #Set intial position
    computer_pad.y = 4 #Number 'of change on the movement' in the Y "axis"   - only vertical movemente on the paddle-
    ...
    while True:                                 #Start game "bucle"
        ...
        turtle.update()                         #Ball movement in 2D (-for better understanding separate the movement cases in which the ball "bounce")
        ...
        computer_pad.sety(computer_pad.ycor() + computer_pad.y)              #Computer pad movement.
                                                                             #Computer pad movements - notes: looking for a rought game beetween player and computer
        ...
        if ball.xcor() < -280:                  #Movement of the ball in the "X edges" of the framework
